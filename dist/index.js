...
function substitutionCipher(text, substitutionTable) {
    let result = '';
    for (let i = 0; i < text.length; i++) {
        let char = text[i];
...
function substitutionDecipher(text, substitutionTable) {
    let result = '';
...
function transpositionDecipher(text, key) {
    ...
    let table = new Array(text.length);
    let idx = 0;
...
function blockCipher(text, blockSize, shift) {
    let result = '';
...
function blockDecipher(text, blockSize, shift) {
    let result = '';
    for (let i = 0; i < text.length; i += blockSize) {
        let block = text.slice(i, i + blockSize);
...
const fs = require('fs');
function fileCipher(filePath, outputPath, shift) {
...
function fileDecipher(filePath, outputPath, shift) {
    fs.readFile(filePath, 'utf8', (err, data) => {
        if (err)
            throw err;
